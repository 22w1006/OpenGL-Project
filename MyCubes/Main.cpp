...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("src/learngl.vs", "src/learngl.fs");
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("resource/cat.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 view = glm::lookAt(Mycamera.Position, Mycamera.Position + Mycamera.Front, Mycamera.Up);
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            float angle = 20.0f * i;
            model = glm::rotate(model, (float)glfwGetTime() * 3 * glm::radians(25.0f + 5 * i), glm::vec3(0.5f + 0.02 * i, ((float)i) / 10, 0.0f));
